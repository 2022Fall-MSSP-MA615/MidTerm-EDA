...
author: "Hao He, Yeyang Han, Tianjian Xie, Yaquan Yang"
date: "2022-11-09"
...
First, read the dataset into Rstudio. After having an overview of all the columns and their data structure, we start with dropping useless columns.
```{r}
# strawb
strawb <- read_xlsx("D:/R dataset/615/Midterm/strawberries-2022oct30-a.xlsx")
...
strawb <- strawb %>% separate(col=`Data Item`,
                    ...
                    fill = "right")
```
We just cleaned the Data Item column, but we still need to take care of Domain and Domain Category columns. 

Before we separate these two columns, we create a plot to see if there are any patterns for chemicals used in different states and we  also have a initial plot of sales for strawberries .
```{r chemicals used in different states echo=FALSE}
...
So far, we have a clean chemicals subset. The number of different chemicals listed in our dataset is 172. Since California contributes to 80% of the strawberry production and followed by Florida and Oregon, we may want to check how many different chemicals are listed by state and if California use more chemicals than other states.
...
```
Based on the tibble above, we can say that California has a higher usage of different chemicals than others which could be one of reasons why California has the highest production.


--------------------
Analyze state in data
 
According to articles, "CALIFORNIA" and "FLORIDA" are majority two places to produce strawberry; "NEW JERSEY", "NEW YORK" and "PENNSYLVANIA" can be grouped together as Northeast and Mid-Atlantic United States (Northeast region); "OREGON"
```{r}
unique(strawb$State)
```
...
All states mentioned in strawberry dataset plant organic strawberry, that means all of these states realize people want to keep healthy and plant relevant things to cater costumers.
-------------------------
...
According to articles, because of the improvement of people's health awareness, organic strawberries will be more and more important in the future. So, we explore organic strawberry subset first.
...
### plot the sales trend for organic strawberry
### plot sales measured in $
```{r echo=FALSE}
ggplot(e1m) +
...
### plot sales measured in CWT
```{r sales measured in CWT, echo=FALSE}
ggplot(e1w) +
  aes(x = Year, y = Value, colour = State) +
  geom_point(shape = "square", size = 3.95) +
  scale_color_hue(direction = 1) +
...
e5 %>%
  ...
  labs(x = "count", y = NULL)
```
Based on our research, safe and natural chemicals to use include Neem Oil and Spinosad. Check if they are in our chemical dataset. 
...
```
Based on the output above, California seems to be cautious about the chemicals 
